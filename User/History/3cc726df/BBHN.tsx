...
const [loading, setLoading] = useState(false);
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (message.trim() && !isLoading && !disabled) {
    ...
    // Call parent's onSend to add user message
    if (onSend) onSend(userMessage);

    try {
      setLoading(true); // show loader in input
      
      console.log("🔄 Calling agent...");
      const reply = await sendToAgent(userMessage); // wait for backend
      
      console.log("✅ Agent reply received");
      // Send agent's response back to parent
      if (onSend) onSend(reply);
      
    } catch (err) {
      console.error("❌ Error in handleSubmit:", err);
      // Send error message back to parent
      if (onSend) onSend("Sorry, something went wrong. Please try again.");
    } finally {
