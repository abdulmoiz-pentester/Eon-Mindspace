...
export function ChatInput({ onSend, isLoading, disabled, onStop }: ChatInputProps) {
  ...
  if (message.trim() && !isLoading && !disabled) {
    ...
    setMessage(""); // clear textarea immediately

    // optional: you can keep isLoading state in parent or here
    if (onSend) onSend(userMessage); // immediately add user's message

    try {
      // call backend
      const reply = await sendToAgent(userMessage);

      // send agent's reply back through onSend too (optional)
